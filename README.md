...
## Arquivos de ação (action files)
são usados ​​para executar ações específicas em um aplicativo ou sistema. Essas ações podem incluir tarefas como criar, atualizar, excluir ou recuperar dados de um banco de dados. Arquivos de ação geralmente contêm funções ou métodos que são invocados em resposta a eventos específicos, como cliques em botões, solicitações de API ou outros eventos de usuário.

## Arquivos de serviço (service files)
são frequentemente usados ​​em sistemas que seguem o padrão de arquitetura de software "Service-Oriented Architecture" (SOA). Esses arquivos contêm a lógica de negócios e as funcionalidades que são expostas como serviços a outros componentes do sistema. Os arquivos de serviço geralmente são usados ​​para encapsular a lógica de negócios e as regras de negócios de uma aplicação em um único lugar, tornando mais fácil para os desenvolvedores manter e modificar o código.

## Arquivos utilitários (util files)
geralmente contêm funções ou métodos que são úteis em várias partes de um aplicativo ou sistema, mas que não pertencem necessariamente a uma classe específica. Essas funções podem incluir, por exemplo, formatação de datas, manipulação de strings, validação de entrada de dados e outras tarefas comuns. Arquivos utilitários geralmente são usados ​​para evitar a repetição de código em diferentes partes do aplicativo e para tornar o código mais fácil de manter. A pasta util geralmente é usada para armazenar funções utilitárias ou de baixo nível que podem ser usadas em diferentes partes do código. Essas funções podem incluir, por exemplo, funções para formatar datas, manipular strings ou realizar cálculos matemáticos simples. A pasta util pode ser usada como um lugar para armazenar código que não se encaixa em outra categoria ou que é projetado para ser usado em muitas partes diferentes do aplicativo.


## Bibliotecas (libs)
são coleções de código reutilizável que podem ser usadas em vários projetos ou aplicativos. As bibliotecas podem ser criadas para fornecer funcionalidades específicas, como manipulação de data, criptografia de dados, interação com APIs ou outras tarefas. As bibliotecas podem ser compartilhadas e reutilizadas por diferentes projetos e equipes, tornando a codificação mais eficiente e consistente. A pasta lib é usada para armazenar código de mais alto nível, como funções que expõem uma interface de programação mais completa ou classes que abstraem serviços externos. A pasta lib pode ser usada como um lugar para armazenar código que é mais central para a lógica de negócios do aplicativo e que é projetado para ser usado por outras partes do código.

## Arquivos auxiliares (helper files)
são semelhantes aos arquivos utilitários, mas geralmente contêm funções ou métodos específicos para auxiliar outras partes do código. Essas funções podem ser usadas para simplificar tarefas complexas ou repetitivas ou para fornecer funcionalidades adicionais a outras partes do sistema.
