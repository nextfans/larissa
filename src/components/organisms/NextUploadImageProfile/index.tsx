...
import { storage, storageRef, uploadBytesResumable, getDownloadURL } from '../../../lib/firebaseClient'
...
const FileUpload = (props: FileUploadProps) => {
  ...
  const handleUploadFile = (file: File) => {
    ...
    const storageRefImage = storageRef(storage, `images/${user?.uid}/${props.directoryName}/${imageCode}`)
...
export const NextUploadImageProfile = ({ directoryName, userData, setUserData}: UploadProps) => {
  const {
    register,
    formState: { errors }
  } = useForm<FormValues>()
  ...
  return (
    <Center h='110px' w='110px' maxH='110px' maxW='110px' border={'4px'} borderStyle={'dashed'} borderColor={'gray.300'} borderRadius={'2xl'}>
      <form>
