...
import { getDownloadURL, storage, storageRef, uploadBytesResumable } from '../../../lib/firebaseClient'
...
const FileUpload = (props: FileUploadProps) => {
  ...
  const [listImages, setListImages] = useState<Array<Record<string, any>> | undefined>()
...
export const NextUpload = ({ id, directoryName }: UploadProps) => {
  ...
  const updateUserVerificationImage = async (downloadURL: string) => {
    ...
    await CallApi({
      ...
      headers: {},
      toastInformation: {
        success: { description: 'A imagem de verificação do perfil foi atualizada com sucesso!' },
        error: { title: 'Tivemos um problema ao atualizar a imagem de verificação do perfil.' }
      },
      user
    })
  }
  ...
  const handleUpload = (file: File) => {
    ...
    const storageRefImage = storageRef(storage, `images/${id}/${directoryName}/${imageCode}`)
    ...
    uploadTask.on(
      ...
      () => {
        ...
        toast({
          ...
          description: 'Agradecemos por enviar sua foto! Enquanto nossa equipe verifica sua conta, você pode aproveitar para treinar o ChatGPT-4 do aplicativo, assim terá melhores matchs. Divirta-se e aprimore suas conexões! ⏳🤖💙',
          status: 'success',
          duration: 12000,
          isClosable: true
        })
